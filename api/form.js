...
module.exports.submit = (event, context, callback) => {
  ...
  if (typeof fullname !== 'string' || typeof phone !=='number' || typeof email !== 'string' || typeof experience !== 'string') {
    console.error('Validation Failed');
    callback(new Error('Couldn\'t submit candidate because of validation errors.'));
    return;
  }

  submitCandidateP(candidateInfo(fullname, phone, email, experience))
    .then(res => {
      callback(null, {
        statusCode: 200,
        body: JSON.stringify({
...
const candidateInfo = (fullname, phone, email, experience) => {
